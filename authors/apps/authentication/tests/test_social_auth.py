...
class TestSocialAuth(TestBaseCase):
    ...
    def test_successful_oauth2_login(self):
        ...
        self.base_200_test(self.oauth2_data, "token")

    def test_successful_oauth1_login(self):
        """test successful oauth1 authentication with twitter"""

        self.base_200_test(self.oauth1_data, "token")
    ...
    def test_login_no_access_token_secret(self):
        """test social login with no access token secret provided"""
        message = "Access token secret is required"
        self.base_400_test(self.no_token_secret_data, "error", message)
